...
from UltraModel import LorentzE,UltraE, RotE
from PoincareModel import RotH, ProjH
...
class Experiment:
        ...
        for i in range(0, len(test_data_idxs), batch):
            ...
            predictions_s = model.forward(
                e1_idx, r_idx, tt[:min(batch,
                                       len(test_data_idxs) - i)],eval_mode=True)
    ...
    def train_and_eval(self):
        ...
        if self.model== "LorentzE":
            model = LorentzE(d, self.dim, self.args.max_scale, self.args.max_norm,
                            self.args.margin)
        elif self.model== "UltraE":
            model = UltraE(d, self.dim, self.args.max_scale, self.args.max_norm,
                            self.args.margin)
        elif self.model== "ProjH":
            model = ProjH(d, self.dim, self.args.max_scale, self.args.max_norm,
                            self.args.margin)
       
        elif self.model=="RotE":
            model = RotE(d, self.dim, self.args.max_scale, self.args.max_norm,
                            self.args.margin)
        ...
        for it in bar:
            ...
            print('Best:%.4f@%d,cur:%.4f,loss:%.3f' %(max_mrr, max_it, mrr, np.mean(losses)))
            model.eval()
            with torch.no_grad():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--margin', default=8, type=float)
    ...
    parser.add_argument('--model',
                        choices=['LorentzE', 'UltraE', 'RotE','UltraJE','RotBoost','ProjH'],
                        default='LorentzE')
