...
class PseudoHyperboloid(Manifold):
    ...
    def expmap(self, x, v, beta, time_dim=None):
        ...
        if True in time_like:
            ...
            U[time_like,:] = torch.clamp(x[time_like,:]*torch.clamp(torch.cosh(beta_product),max=self.max_norm) +  torch.clamp( torch.clamp(v[time_like,:]*torch.sinh(beta_product), max=self.max_norm)/beta_product,  max=self.max_norm),max=self.max_norm)
            # print(beta_product.max().item(),beta_product.min().item() )
    ...
    def to_sphere_R(self, x, beta, time_dim=None):
        ...
        if time_dim == self.dim:
            ...
            Xtime = torch.clamp(F.normalize(x[:,0:time_dim]),max=self.max_norm)*(beta.abs()**0.5)
    ...
    def logmap0(self, x, beta, time_dim=None):
        ...
        tangent_s = self.logmap_0(sphere_r[:,0:time_dim],beta, time_dim=time_dim)
        tangent_r = sphere_r[:,time_dim:]
        return torch.cat((tangent_s, tangent_r),1)

    def expmap0(self, x, beta, time_dim=None):
        time_dim = self.time_dim if time_dim==None else time_dim
        tangent_s = x[:,0:time_dim]
        tangent_r = x[:,time_dim:]
        # tangent_s = self.proj_tan0(tangent_s,beta,time_dim=0)
        x_sphere = self.expmap_0(tangent_s, beta, time_dim=time_dim)
        x_euclidean = tangent_r
        sphere_r = torch.cat((x_sphere, x_euclidean),1)
        U = self.from_sphere_R(sphere_r, beta, time_dim)
        return U
    ...
    def logmap_n(self, x, y, beta, time_dim=None):
        ...
        if True in time_like_positive:
            ...
            # print(beta.item(), beta_product_positive[time_like_positive].max().item(),beta_product_positive[time_like_positive].min().item())
            # print(beta_product_positive[time_like_positive].max())
            assert not torch.isnan(up).any()
            assert not torch.isnan(low).any()
    ...
    def logmap(self, x, y, beta, time_dim=None):
        ...
        if True in negative_log_map:
            ...
            assert False
            # y[negative_log_map] = self.proj_tan(x[negative_log_map],y[negative_log_map])
            # y[negative_log_map] = self.expmap(x[negative_log_map],y[negative_log_map], beta)
            # U[negative_log_map] = self.logmap_n(x[negative_log_map], y[negative_log_map], beta, time_dim=time_dim)

            # y[negative_log_map] = self.expmap(x[negative_log_map],y[negative_log_map], beta)
            # U[negative_log_map] = self.logmap_n(x[negative_log_map], y[negative_log_map], beta, time_dim=time_dim)
            # U[negative_log_map][:,0] = y[negative_log_map][:,0]
    ...
    def logmap_0(self,y, beta, time_dim=None):
        ...
        origin[:,:] = 0
        origin[:,0] = abs(beta)**0.5
        # print(self.inner(origin,y).max().item(),self.inner(origin,y).min().item())
    ...
    def proj(self, x, beta, time_dim=None):
        ...
        Xspace = torch.clamp(x[:,time_dim:].div((beta.abs())**0.5),max=self.max_norm)
        spaceNorm = torch.clamp(torch.sum(Xspace*Xspace, dim=1, keepdim=True),max=self.max_norm)
        if self.time_dim == 1:
            Xtime = torch.sqrt((spaceNorm).add(1.0)).view(-1,1)
        else:
            Xtime = torch.clamp(torch.clamp(torch.sqrt(spaceNorm.add(1.0)),max=self.max_norm).expand_as(Xtime) * Xtime, max=self.max_norm)
        U =  torch.cat((Xtime,Xspace),1)*(beta.abs()**0.5)
        return U
    ...
    def proj_tan0(self, z, beta, time_dim=None):
        ...
        origin[:,0] = abs(beta)**0.5
        U = z.clone()
        U[:,0:time_dim] = self.proj_tan(z[:,0:time_dim], origin[:,0:time_dim], beta, time_dim=time_dim)
        U[:,time_dim:] = z[:,time_dim:]
        return U
    ...
    def mobius_matvec(self, m, x, beta, time_dim=None):
        ...
        # mu = F.normalize(mu)
        mu = self.proj_tan0(mu, beta, time_dim=time_dim)
        mu = self.expmap0(mu, beta, time_dim=self.time_dim)
        mu = self.perform_rescaling_beta(mu, beta, time_dim=time_dim)
        return mu

    # def mobius_add(self, x, y, beta):
    #     u = self.logmap_0(y,beta)
    #     v = self.ptransp0(x,u,beta)
    #     return self.expmap(x,v,beta)

    def to_sphere(self, x, beta,time_dim=None):
        time_dim = self.time_dim if time_dim==None else time_dim
        sphere_projection = x.clone()
        sphere_projection[:,0:time_dim] = torch.clamp(F.normalize(x[:,0:time_dim]),max=self.max_norm)
        sphere_projection[:,time_dim:] = 0
