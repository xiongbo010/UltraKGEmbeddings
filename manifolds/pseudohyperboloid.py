...
class PseudoHyperboloid(Manifold):
    ...
    def expmap(self, x, v, beta, time_dim=None):
        ...
        if True in null_geodesic:
            # print('exp:null_like')
            # print(x[null_geodesic,:].max().item(), v[null_geodesic,:].max().item())
    ...
    def projx(self, x, beta, time_dim=None):
        time_dim = self.time_dim if time_dim==None else time_dim
        # self.beta = self.beta.cuda().to(x.get_device())
    ...
    def mobius_matvec(self, m, x, beta, time_dim=None):
        ...
        mu = self.perform_rescaling_beta(mu,beta)
        return mu

    # def to_poincare(self, x, beta, time_dim=None):
    #     # to hyperboloid
    #     time_dim = self.time_dim if time_dim==None else time_dim
    #     res = x.clone()
    #     res[:,0] = (x[:,0]**2 + 2*torch.norm(x[:,1:time_dim],p=2, dim=1)**2).sqrt()
    #     K = beta.abs()
    #     sqrtK = K ** 0.5
    #     d = res.size(-1) - 1
    #     res = sqrtK * res.narrow(-1, 1, d) / (res[:, 0:1] + sqrtK)
    #     return res


    # def from_poincare(self, x, beta,time_dim=None):
    #     time_dim = self.time_dim if time_dim==None else time_dim
    #     K = beta.abs()
    #     sqrtK = K ** 0.5
    #     sqnorm = torch.norm(x, p=2, dim=1, keepdim=True) ** 2
    #     x = sqrtK * torch.cat([K + sqnorm, 2 * sqrtK * x], dim=1) / (K - sqnorm)
    #     res = x.clone()
    #     res[:,0] = (x[:,0]**2 - 2*torch.norm(x[:,1:time_dim],p=2, dim=1)**2).sqrt()
    #     return res

    # def to_hyperboloid(self, x, beta, time_dim=None):
    #     # to hyperboloid
    #     time_dim = self.time_dim if time_dim==None else time_dim
    #     res = x.clone()
    #     res[:,0] = (x[:,0]**2 + 2*torch.norm(x[:,1:time_dim],p=2, dim=1)**2).sqrt()
    ...
    # def from_hyperboloid(self, x, beta,time_dim=None):
    #     time_dim = self.time_dim if time_dim==None else time_dim
    #     res = x.clone()
    #     res[:,0] = (x[:,0]**2 - 2*torch.norm(x[:,1:time_dim],p=2, dim=1)**2).sqrt()
    #     return res

    # def non_linear_activation(self, x, beta, activation, time_dim=None):
    ...
    # def mobius_add(self, x, y, beta):
    #     z = self.proj_tan(x,y)
    #     res = self.expmap(x, z, beta)
    #     res = self.perform_rescaling_beta(mu,beta)
    #     return 

    #     # x_h = self.to_hyperboloid(x, beta)
    #     # y_h = self.to_hyperboloid(y, beta)
    #     # c = 1. / beta.abs()
    #     # z_h = self.hyperboloid.mobius_add(x_h,y_h, c)
    #     # z = self.from_hyperboloid(z_h, beta)
    #     tan_y = self.proj_tan(x, y)
    #     bias = self.expmap(x, tan_y, beta)
        # return self.perform_rescaling_beta(res,beta)
