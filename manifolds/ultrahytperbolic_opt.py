...
class PseudoHyperboloid(Manifold):
    ...
    def expmap(self, x, v, time_dim=None):
        ...
        norm_product = torch.sqrt(torch.clamp(inner,min=self.min_norm).abs())
    ...
    def logmap(self,x,y, time_dim=None):
        ...
        if True in negative_log_map:
            print("log:negative")
            # reference = y[negative_log_map]
            # reference[:,time_dim:] = 0
            # reference[:,0] = 0
            # inner = self.inner(reference,reference,time_dim=time_dim)
            # reference_point = self.beta.abs().sqrt()*reference/(inner.abs().sqrt()).unsqueeze(1)
    ...
    #     reference_point = self.beta.abs().sqrt()*reference/(inner.abs().sqrt()).unsqueeze(1)
    #     median = self.logmap_n(reference_point, y, time_dim=time_dim)
    #     U = self.ptransp(reference_point, x, median, time_dim=time_dim)
    ...
    # reference_point = self.beta.abs().sqrt()*reference/(inner.abs().sqrt()).unsqueeze(1)
    # median = self.logmap_n(reference_point, y, time_dim=time_dim)
    # U = self.ptransp(reference_point, origin, median, time_dim=time_dim) 
    ...
    def mobius_add(self, x, y):
        ...
        u = self.logmap0(y)
        # assert self._check_vector_on_tangent0(u)
        v = self.ptransp0(x,u)
        # assert self._check_vector_on_tangent(x,v)
        # print("mobius add:",x.max(),u.max(),v.max())
    ...
    def ptransp_n(self,x,y,u,time_dim=None):
        ...
        # print(self.inner(y,U).max(), self.inner(y,U).min())
            # print("null",U[null_like,:].max())
        # U = self.proj_tan(y,U)
        # print(U.max())
        return U
