...
class PseudoHyperboloid(Manifold):
    ...
    #     return torch.clamp(dist2, min=0.00001,max=100)
    ...
    def sqdist(self, x, y, beta, time_dim=None):
        ...
        time_dist = torch.nn.functional.pairwise_distance(Xtime,Ytime_scaling, p=2.0, keepdim=True)  
        space_dist = torch.nn.functional.pairwise_distance(Xspace, Yspace, p=2.0, keepdim=True)  
        dist = time_dist + space_dist
        # print('dist:',time_dist.max().item(),space_dist.max().item(),dist.shape)
