...
class PseudoHyperboloid():
    ...
    def inner(self, x, y, time_dim=None):
        ...
        z = x * y
        res = z[:,time_dim:].sum(dim=-1) - z[:, 0:time_dim].sum(dim=-1)
        return res
    ...
    def sqdist(self, x, y, beta, time_dim=None):
        ...
        if True in c4: # cycle + hierachial
            d = torch.min(self.cycle_dist(x[c4], beta) + self.sqdist(-x[c4], y[c4], beta), self.cycle_dist(y[c4],beta) + self.sqdist(x[c4], -y[c4], beta))
            dist2[c4] = d

        if True in c5: # cycle
            dist2[c5] = ((beta.abs()**0.5)*math.pi)**2

        # print(dist2[c1].shape[0],dist2[c3].shape[0],dist2[c4].shape[0],dist2[c5].shape[0],dist2[c2].shape[0]) # hyperbo, spherical, mixed
        return dist2

    def cycle_dist(self,x,beta):
        return (beta.abs()**0.5)*math.pi #* x.norm(dim=1,p=2)

    def sphere_dist(self, x: torch.Tensor, y: torch.Tensor, K, keepdim=False) -> torch.Tensor:
        inner = (x * y).sum(-1, keepdim=keepdim)
        k_inner= torch.clamp(K*inner,min=-1.0,max=1.0)
        a = torch.clamp(torch.acos(k_inner),min=0.00001,max=50)
        return torch.clamp(a/(K.sqrt()),min=0.00001,max=50)

    def hyperbolic_dist(self, x, y, beta, keepdim=False) -> torch.Tensor:
        c = -beta
        sqrt_c = c ** 0.5
        ...
        pairwise_norm = num / denom
        dist = artanh(sqrt_c * pairwise_norm)
        return 2 * dist / sqrt_c

    def manhattan_sqdist(self, x, y, beta, time_dim=None):
        time_dim = self.time_dim if time_dim==None else time_dim
        Xtime = x[:,0:time_dim]
        Ytime = y[:,0:time_dim]
        Xspace = x[:,time_dim:]
        ...
        XtimeNorm = torch.clamp(torch.sum(Xtime*Xtime, dim=1, keepdim=True),max=self.max_norm)
        YtimeNorm = torch.clamp(torch.sum(Ytime*Ytime, dim=1, keepdim=True),max=self.max_norm)
        K = torch.clamp(1/XtimeNorm,min=self.min_norm)
        scaling = XtimeNorm/YtimeNorm
        Ytime_scaling = Ytime*(scaling.sqrt())
        Yspace_scaling = Yspace*(scaling.sqrt())
        time_dist = self.sphere_dist(Xtime,Ytime_scaling, K, keepdim=True)
        # space_dist = self.hyperbolic_dist(Xspace,Yspace_scaling, K, keepdim=True)
        space_dist = (Xspace-Yspace_scaling).norm(dim=1,p=2,keepdim=True)
        dist = time_dist + space_dist
        return torch.clamp(dist, min=0.00001,max=50)
    ...
    def expmap(self, x, v, beta, time_dim=None):
        ...
        if True in null_geodesic:
            # print('exp:null_like')
            U[null_geodesic,:] = torch.clamp( x[null_geodesic,:] + v[null_geodesic,:], min=self.min_norm, max=self.max_norm)
